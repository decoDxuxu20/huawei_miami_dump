...
def PrintIntStack(infile, field, offset, slen, version, mode, outfile):
        addr = 0
        row = 0
        i = 0
        MyOffset = eval(offset)
        infile.seek(MyOffset)
        print("got entry 0x2000080 PrintIntStack")
...
def entry_0x2000080(infile, field, offset, slen, version, mode, outfile):
        try:
            if not field == '0x2000080':
                print('hidis field is ', field)
                print('current field is', '0x2000080')
            ...
            print("got entry 0x2000080")
            #outfile.writelines("============INT STACK1============\n")
            #PrintIntStack(infile, field, offset, slen, version, mode, outfile)
            return 0
