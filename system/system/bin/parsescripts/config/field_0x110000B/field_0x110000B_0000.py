...
def PrintBaseInfo(infile, field, offset, slen, version, mode, outfile):
        ...
        outfile.writelines("taskName           :%s\n" % g_taskName.decode(encoding="utf-8").strip('\x00').replace('\x00',''))
        ...
        outfile.writelines("version               :%s\n" % g_version.decode(encoding="utf-8").strip('\x00').replace('\x00',''))
        outfile.writelines("compile_time      :%s\n" % g_compile_time.decode(encoding="utf-8").strip('\x00').replace('\x00',''))
        return
 	
def entry_0x110000B(infile, field, offset, slen, version, mode, outfile):
        try:
            if not field == '0x110000B':
                print('hidis field is ', field)
                print('current field is', '0x110000B')
            ...
            print("got entry 0x110000B")
            outfile.writelines("============MODEMAP BASEINFO============\n")
