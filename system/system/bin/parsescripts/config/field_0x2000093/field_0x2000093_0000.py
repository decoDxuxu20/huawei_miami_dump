...
def PrintWdtData(infile, field, offset, slen, version, mode, outfile):
        addr = 0
        row = 0
        i = 0
        MyOffset = eval(offset)
        infile.seek(MyOffset)
        print("got entry 0x2000093 PrintWdtData")
...
def entry_0x2000093(infile, field, offset, slen, version, mode, outfile, field_list = None):
        try:
            if not field == '0x2000093':
                print('hidis field is ', field)
                print('current field is', '0x2000093')
            ...
            print("got entry 0x2000093")
            outfile.writelines("============WDT============\n")
            PrintWdtData(infile, field, offset, slen, version, mode, outfile)
            return 0
